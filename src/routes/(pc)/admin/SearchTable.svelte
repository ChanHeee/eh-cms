...
<div class={"flex text-sm border-l mb-7 overflow-scroll " + classString}>
  <div class="flex flex-col h-fit whitespace-nowrap border-r divide-y border-b">
    <div
      id="nameField"
  ...
  <div
    class="flex flex-col h-fit flex-none whitespace-nowrap border-r divide-y border-b"
  >
    <button
    ...
    {#each seongdos as item, index}
      <div class="flex items-center px-3 h-10">
        <input
          ...
          class="w-[8rem] placeholder:text-xs w-full border-gray-300 border text-gray-900 text-sm focus:outline-0 px-2 py-1"
          placeholder="구분"
        />
      </div>
    {/each}
