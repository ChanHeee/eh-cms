...
>
  ...
  <!-- <div class="flex h-full flex-col w-full-if-mobile">
    
    <div class="mb-7 overflow-scroll">
          ...
          {#each seongdosSlice as item, index}
            <div class="flex items-center px-3 h-10">
              <button
                ...
                class="flex items-center gap-1 rounded-sm text-white text-xs px-2 py-[0.4rem]"
                on:click={async () => {
                  seongdoIds = seongdoIds.filter(
                    (seongdo) => seongdo != item._id
                  )
                  await submitHandler()
                  seongdos = seongdos.filter(
                    (seongdo) => seongdo._id != item._id
                  )
                }}
              >
                <TrashCan fill="#4a4a4a" size={20} />
    ...
    <div class="pagination w-fit mx-auto flex items-center">
      {#if total > 0}
        <button
          on:click={async () => {
            skip = pages[0].skip
