...
<div class="overflow-scroll flex text-sm mb-7 border-l bg-white">
  <!-- <div class="flex flex-col whitespace-nowrap border-r divide-y border-b">
    <div class=" flex justify-center bg-[#D9D9D8] font-bold items-center h-10">
      사진
    </div>
  ...
  <div class="flex flex-col whitespace-nowrap border-r divide-y border-b">
    <button
      ...
      on:focus={null}
      on:click={() => {
        const { order, take, className, ...rest } = searchParams
        let newOrder
        if (order == "ageAsc") {
          newOrder = "ageDesc"
  ...
  <div class="flex flex-col whitespace-nowrap border-r divide-y border-b">
    ...
    {#each seongdoEdus as item}
      <button
        ...
        on:click={() => {
          if (item.seongdo.group1) {
            goto(
              `/seongdos${getSearchParams({
                group1: item.seongdo.group1,
                group2: item.seongdo.group2,
              })}`
            )
          }
        }}
